...
export default function EarthMap({ assets, selectedId, activeLayers, onSelect }: EarthMapProps) {
  ...
  const nearest = useMemo(() => selected ? assets.filter((asset) => asset.id !== selected.id).map((asset) => ({ asset, km: 6371 * 2 * Math.asin(Math.sqrt(Math.sin(((asset.geo.latitude - selected.geo.latitude) * Math.PI / 180) / 2) ** 2 + Math.cos(selected.geo.latitude * Math.PI / 180) * Math.cos(asset.geo.latitude * Math.PI / 180) * Math.sin(((asset.geo.longitude - selected.geo.longitude) * Math.PI / 180) / 2) ** 2)) })).sort((a, b) => a.km - b.km)[0] : undefined, [assets, selected]);
  ...
  return <section className="relative min-h-[590px] overflow-hidden rounded-2xl border border-white/10 bg-[#070807] shadow-[0_40px_120px_rgba(0,0,0,.6)]" aria-label="Interactive synthetic Earth assessment map">
    <MapContainer center={[25, 8] as LatLngExpression} zoom={2} minZoom={2} maxZoom={15} zoomControl className="absolute inset-0 z-0 h-full w-full" attributionControl={false}>
      ...
      {heatmap && assets.filter((asset) => asset.criticality >= 6).map((asset) => <Circle key={`heat-${asset.id}`} center={[asset.geo.latitude, asset.geo.longitude]} radius={(asset.criticality - 4) * 130000} pathOptions={{ color: "#ee7664", fillColor: "#ee7664", fillOpacity: 0.055, weight: 0, opacity: 0 }} />)}
    ...
    <div className="absolute left-4 top-[104px] z-[400] w-[190px] rounded-lg border border-white/12 bg-[#080705]/86 p-3 backdrop-blur-md"><p className="eyebrow">SPATIAL ANALYSIS</p><label className="mt-2 block text-[10px] text-white/45">Buffer · {bufferKm} km<input className="mt-1 w-full accent-[#e8b760]" type="range" min="10" max="150" step="5" value={bufferKm} onChange={(event) => setBufferKm(Number(event.target.value))}/></label><button onClick={() => setHeatmap((value) => !value)} className={`mt-2 w-full rounded border px-2 py-1.5 text-left text-[10px] ${heatmap ? "border-[#e8b760]/35 bg-[#e8b760]/[.08] text-[#f2cb82]" : "border-white/10 text-white/48"}`}>Risk concentration heat · {heatmap ? "visible" : "hidden"}</button><p className="mt-2 text-[10px] leading-4 text-white/50">{nearest ? `Nearest synthetic context: ${nearest.asset.name} · ${nearest.km.toFixed(0)} km great-circle distance` : "Select a record for distance context."}</p></div>
  </section>;
}
